...
esp_err_t context_build_system_prompt(char *buf, size_t size)
{
    ...
    off += snprintf(
        ...
        "- read_temperature_humidity: Read temperature and humidity from the AHT10 I2C sensor. Prefer this when the user asks about room temperature, humidity, AHT10, 温度, 湿度, or 温湿度.\n"
        ...
        "Allowed GPIO pins for user control: 1-18, 21, 38, 46. GPIO19/20 are reserved for USB Serial/JTAG. GPIO0 is the boot button. The servo_write tool uses GPIO " MIMI_STRINGIFY(MIMI_SERVO_DEFAULT_GPIO) " (fixed, no pin override). WS2812 RGB LED is on GPIO48. SGP30 sensor uses configured SDA=GPIO " MIMI_STRINGIFY(MIMI_SGP30_DEFAULT_SDA_GPIO) "/SCL=GPIO " MIMI_STRINGIFY(MIMI_SGP30_DEFAULT_SCL_GPIO) ". GY-30/BH1750 light sensor defaults to SDA=GPIO " MIMI_STRINGIFY(MIMI_BH1750_DEFAULT_SDA_GPIO) "/SCL=GPIO " MIMI_STRINGIFY(MIMI_BH1750_DEFAULT_SCL_GPIO) ". AHT10 temperature/humidity sensor defaults to SDA=GPIO " MIMI_STRINGIFY(MIMI_AHT10_DEFAULT_SDA_GPIO) "/SCL=GPIO " MIMI_STRINGIFY(MIMI_AHT10_DEFAULT_SCL_GPIO) ". A 3-wire presence sensor uses VCC/GND/OUT with OUT on the configured presence GPIO. HC-SR05 uses configured Trig/Echo pins and Echo must be level-shifted or divided before ESP32 input if the module outputs 5V.\n\n"
