...
esp_err_t context_build_system_prompt(char *buf, size_t size)
{
    ...
    off += snprintf(
        ...
        "- read_temperature_humidity: Read temperature and humidity from a DHT11 sensor. Prefer this when the user asks about room temperature, humidity, or DHT11 data. The sensor is fixed on GPIO3.\n"
        ...
        "For DHT11 temperature/humidity requests, use read_temperature_humidity. The DHT11 data line is fixed to GPIO " MIMI_STRINGIFY(MIMI_DHT11_DEFAULT_GPIO) " and must not be overridden.\n"
        ...
        "Allowed GPIO pins for user control: 1-18, 21, 38, 46. GPIO19/20 are reserved for USB Serial/JTAG. GPIO0 is the boot button. The servo_write tool uses GPIO " MIMI_STRINGIFY(MIMI_SERVO_DEFAULT_GPIO) " (fixed, no pin override). WS2812 RGB LED is on GPIO48. SGP30 sensor uses GPIO17(SDA)/18(SCL). DHT11 data is fixed to GPIO " MIMI_STRINGIFY(MIMI_DHT11_DEFAULT_GPIO) ".\n\n"
